...
# def clearml_log_summary(epoch, train_metrics, eval_metrics):
#     Logger.current_logger().report_scalar("losses", "train", iteration=epoch, value=train_metrics['loss'])
#     Logger.current_logger().report_scalar("losses", "test", iteration=epoch, value=eval_metrics['loss'])

#     Logger.current_logger().report_scalar("metrics", "top1", iteration=epoch, value=eval_metrics['top1'])
#     Logger.current_logger().report_scalar("metrics", "top5", iteration=epoch, value=eval_metrics['top5'])
...
group.add_argument(
    ...
    action="store_true",
    default=False,
    help="log training and validation metrics to ClearML",
)
group.add_argument(
    "--project-name", default="", type=str, metavar="NAME", help="name of project"
)
...
def main():
    ...
    #         output_uri=True,
    #         # output_uri='s3://plantin-clearml-storage',
    #         auto_connect_frameworks={'pytorch': False}
    #     )
    #     task.set_base_docker(
    #         "902342342/timm-models:latest",
    #         docker_arguments='--ipc=host -e="CLEARML_AGENT_SKIP_PYTHON_ENV_INSTALL=1" -v /home/gpubox2/datasets/final-mushroom-dataset:/data/final-mushroom-dataset',
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if output_dir is not None:
                ...
                # if args.log_clearml:
                #     clearml_log_summary(epoch, train_metrics, eval_metrics)
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                ...
                #     "learning rate", "lr", iteration=num_updates, value=lr
                # )
                # Logger.current_logger().report_scalar(
                #     "general", "train_loss", iteration=num_updates, value=losses_m.val
                # )

                # Logger.current_logger().report_scalar(
                #     "perfomance/update_time",
                #     "update_time_s",
                #     iteration=num_updates,
                #     value=update_time_m.val,
                # )
                # Logger.current_logger().report_scalar(
                #     "perfomance/update_samples",
                #     "update_samples_s",
                #     iteration=num_updates,
                #     value=(update_sample_count / update_time_m.val),
                # )
